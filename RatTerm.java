...
public final class RatTerm {
	...
	public RatTerm add(RatTerm arg) {
		...
		if (arg != null) {
			if (this.isNaN() || arg.isNaN()) {
				...
				// if we initially had 0, we should take the exponent from
				// arg.expt
			} else if (this.isZero()) {
				return new RatTerm(this.coeff.add(arg.getCoeff()), arg.expt);
				// if we did not initially have 0, we can safely take the
				// exponent from this.expt
			} else {
	...
	public RatTerm div(RatTerm arg) {
		if (arg != null) {
			...
			// divide the coefficient and subtract the exponent of the term
			// being divided
			else {
	...
	public RatTerm differentiate() {
		...
		// multiply the coefficient with the exponent and subtract one from the
		// exponent
		if (this.expt > 0) {
