...
public final class RatTerm {
  ...
  public boolean isNaN() {
    //check if the coefficient is NaN
	  if(this.coeff.equals(RatNum.NaN)){
		  return true;
	  }
	  else{
		  return false; 
  ...
  public double eval(double d) {
    //if NaN return NaN, else evaluate the value and return
	  if(this.isNaN() == true){
		  return Double.NaN;
	  }else{
  ...
  public RatTerm negate() {
     //if NaN return NaN, else negate the value and return
	  if(this.isNaN()){
		  return NaN;
	  }else{
  ...
  public RatTerm add(RatTerm arg) {
	  ...
	  if(arg!=null){
		  if(this.isNaN() || arg.isNaN()){
			  return NaN;
		  }else if(this.expt != arg.expt && !(arg.isZero() || this.isZero() || arg.isNaN() || this.isNaN())){
			  throw new IllegalArgumentException();
		 //if we initially had 0, we should take the exponent from arg.expt
		  }else if(this.isZero()){
			   return new RatTerm(this.coeff.add(arg.getCoeff()), arg.expt);
	     //if we did not initially have 0, we can safely take the exponent from this.expt
		  }else{
  ...
  public RatTerm sub(RatTerm arg) {
	  if(arg!=null){
		  ...
		  //if we initially had 0, we should take the exponent from arg.expt
			   ...
			   return new RatTerm(this.coeff.sub(arg.getCoeff()), arg.expt);
		  //if we did not initially have 0, we can safely take the exponent from this.expt
		  }else{
  ...
  public RatTerm mul(RatTerm arg) {
	  if(arg!=null){
		  if(this.isNaN() || arg.isNaN()){
			  return NaN;
		  //multiply the coefficient of both terms and add the exponents
		  }else{
  ...
  public RatTerm div(RatTerm arg) {
	  if(arg!=null){
		  ...
		  //divide the coefficient and subtract the exponent of the term being divided
		  else{
  ...
  public RatTerm differentiate() {
	  ...
	  //multiply the coefficient with the exponent and subtract one from the exponent
	  if(this.expt > 0 ){
		  return new RatTerm(this.coeff.mul(new RatNum(this.getExpt(),1)), this.expt - 1);
	  }else{
		  return RatTerm.ZERO;
	  }
  }
  ...
  public RatTerm antiDifferentiate() {
	  ...
	  //add one to the exponent, and divide the coefficient with that number
	  if(this.expt >= 0 ){
		  return new RatTerm(this.coeff.div(new RatNum(this.getExpt()+1,1)), this.expt + 1);
      }else{
		  return RatTerm.ZERO;
	  }
  }
