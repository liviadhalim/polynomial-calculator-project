...
public final class RatPoly {
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
      ...
      for (int i = 0; i < lst.size(); i++) {
    	  ...
    	  //if exponent of new Term is bigger, there must not exist a term with the same coefficient, so we add it into the list.
  	  ...
  	  //we end up adding the term if the list has ended and we have not found a term that matches the newTerm, 
  ...
  public RatPoly mul(RatPoly p) {
  	...
  	for(int i = 0; i< this.terms.size(); i++){
    	...
    	//put a copy of p to the new temporary arrayList
        temp.addAll(p.terms);
  		//multiply the coefficient of the terms in temp with the coefficient of the terms in this
  		scaleCoeff(temp, this.terms.get(i).getCoeff());
  		//add the exponents of the terms in temp with the exponents of the terms in this
  ...
  public RatPoly div(RatPoly p) {
    ...
    //RatPoly to store the copy of this
  	...
  	//check that the remainder is not 0 and the degree is bigger than the divider's degree
  	while (r.terms.size() != 0 && r.degree() >= p.degree()) {
  		//divide the first term's coefficient in r with the first term's coefficient in p 
  		RatNum c = r.terms.get(0).getCoeff().div(p.terms.get(0).getCoeff());
  		//subtract the first term's degree in r with the first term's degree in p
  		RatPoly t = new RatPoly(new RatTerm(c, r.degree() - p.degree()));
  		//add the term to the result of the division
  		q = q.add(t);
  		//subtract the polynomial factor from the polynomial being divided by p 
  		r = r.sub(t.mul(p));
  	}
      q.checkRep();
      return q;
  ...
  public RatPoly differentiate() {
		if(!this.isNaN()){
	    	...
	    	for (int i = 0; i < temp.size(); i++) {
	    		//if exponent is 0, remove the item because we dimiss a derivative of a constant
  ...
  public double integrate(double lowerBound, double upperBound) {
	  if (this.isNaN() || Double.isNaN(lowerBound) || Double.isNaN(upperBound))
  	  ...
  	  for(int i = 0 ; i<r.terms.size(); i++){
  		  //store the sum of the integrated polynomials that have been evaluated in upperBound
			sum1 += r.terms.get(i).getCoeff().doubleValue()*Math.pow(upperBound, r.terms.get(i).getExpt()); 
		  //store the sum of the integrated polynomials that have been evaluated in lowerBound
