...
/**
 * <b>PolyGraph</b> is part of the graphical calculator that utilizes all of the
 * other classes in package hw4. PolyGraph implements the graphing component.
 *
...
public final class PolyGraph extends Canvas {
	...
	public void paint(Graphics g) {
		...
		if (calcFrame.stack.size() > 1) {
			...
			updatePlotBuffer(xMin, xMax, numIncrements, xValBuffer2, yValBuffer2, yExtrema, currentRatPoly);
		...
		if (calcFrame.stack.size() > 2) {
			...
			updatePlotBuffer(xMin, xMax, numIncrements, xValBuffer3, yValBuffer3, yExtrema, currentRatPoly);
		...
		if (calcFrame.stack.size() > 3) {
			...
			updatePlotBuffer(xMin, xMax, numIncrements, xValBuffer4, yValBuffer4, yExtrema, currentRatPoly);
		...
		if (calcFrame.stack.size() > 1) {
			assert xValBuffer2 != null : "@SuppressWarnings(nullness): dependent:  non-null if calcFrame.stack.size() > 1";
			assert yValBuffer2 != null : "@SuppressWarnings(nullness): dependent:  non-null if calcFrame.stack.size() > 1";
			drawPlot(xMin, xMax, yMin, yMax, xValBuffer2, yValBuffer2, gw, gh, bord, numIncrements, h, g);
		}

		g.setColor(Color.green);
		if (calcFrame.stack.size() > 2) {
			assert xValBuffer3 != null : "@SuppressWarnings(nullness): dependent:  non-null if calcFrame.stack.size() > 2";
			assert yValBuffer3 != null : "@SuppressWarnings(nullness): dependent:  non-null if calcFrame.stack.size() > 2";
			drawPlot(xMin, xMax, yMin, yMax, xValBuffer3, yValBuffer3, gw, gh, bord, numIncrements, h, g);
		}

		g.setColor(Color.orange);
		if (calcFrame.stack.size() > 3) {
			assert xValBuffer4 != null : "@SuppressWarnings(nullness): dependent:  non-null if calcFrame.stack.size() > 3";
			assert yValBuffer4 != null : "@SuppressWarnings(nullness): dependent:  non-null if calcFrame.stack.size() > 3";
	...
	public void updatePlotBuffer(float xMin, float xMax, int numIncrements, float xValBuffer[], float yValBuffer[],
	...
	public void drawPlot(float xMin, float xMax, float yMin, float yMax, float xValBuffer[], float yValBuffer[],
			float gw, float gh, int bord, int numIncrements, int h, Graphics g) {
		float xVal = 0;
		float yVal = 0;
		float previousX = 0;
		float previousY = 0;
